...
def get_library(db: Session = Depends(get_db)):
    ...
    files = db.scalars(
        select(LibraryFile).order_by(LibraryFile.created_at.desc())
    ).all()

    return LibraryData(
        folders=[folder_out(folder) for folder in folders],
        files=[file_out(file) for file in files]
    )
...
def update_file(
    ...
    for field, value in payload.model_dump(exclude_none=True).items():
        setattr(file, mapping[field], value)
...
def get_stats(db: Session = Depends(get_db)):
    ...
    storage_used = db.scalar(
        select(func.sum(LibraryFile.size))
    ) or 0
